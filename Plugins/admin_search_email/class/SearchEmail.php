  ...
  function ajax_search_email()
  {
    ...
    if($user!=array()) {
      if(function_exists('osc_csrf_token_url')) {
        $aData  .= '<div><div style="width:150px; display:inline-block;"><a class="btn" target="_black" style="margin-right:15px;" href="' . osc_admin_base_url(true) . '?page=users&action=delete&amp;id[]=' . $user['pk_i_id'] . '&amp;' . osc_csrf_token_url() . '">' . __('Delete user', 'admin_search_email') . '</a></div><span style=" display: inline-block; top: -14px;position: relative;">'.__('Found a user with this email','admin_search_email').'</span></div>';
      } else {
        $aData  .= '<div><div style="width:150px; display:inline-block;"><a class="btn" target="_black" style="margin-right:15px;" href="' . osc_admin_base_url(true) . '?page=users&action=delete&amp;id[]=' . $user['pk_i_id'] . '">' . __('Delete user', 'admin_search_email') . '</a></div><span style=" display: inline-block; top: -14px;position: relative;">'.__('Found a user with this email','admin_search_email').'</span></div>';
      }
    } else {
    ...
    $aData .= '<div>';
    if($alerts!=array()) {
      $aData  .= '<div><div style="width:150px; display:inline-block;"><a class="btn" target="_black" style="margin-right:15px;" href="' . osc_admin_base_url(true) .'?page=ajax&action=runhook&hook=lai_delete_alert' . '&s_email='.$email.'"><i class="circle circle-red btn-red"> <b>'.count($alerts).'</b> </i>' . __('Delete alerts', 'admin_search_email') . '</a></div><span style=" display: inline-block; top: -14px;position: relative;">'.__('Found alerts with this email','admin_search_email').'</span></div>';
    } else {
      $aData .= '<span>'.__('There is no alerts with this email', 'admin_search_email').'</span>';
    }
    $aData .= '</div><div style="clear:both;"></div></br>';
    unset($alerts);
    ...
    if($total>0) {
      $manage_listings_url = '';
      $aData .= '<span>'. sprintf(__('%s listings with email', 'admin_search_email'), '<b>'.$total.'</b>').' '.$email.'</span> <a href="'.osc_admin_base_url(true).'?page=items&user='.urlencode($email).'"> ( '.__('View all listings', 'admin_search_email').' )</a>';
    } else {
      $aData .= '<span>'.__('There is no listings with this email', 'admin_search_email').'</span>';
    }
