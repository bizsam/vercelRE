<?php
/*
  Plugin Name: XML Ad Importer Plugin
  ...
  Short Name: xml_importer
  Plugin update URI: xml_importer
  Support URI: https://forums.osclasspoint.com/free-plugins/
  Product Key: bgX4ETvrv6sIfluntcwz
*/

function xmlimporter_admin_menu() {
  osc_add_admin_submenu_page(
    'plugins',
    __('Ad Importer', 'xmlimporter'),
...
function xmlimporter_countads($file) {
  $xml = new DOMDocument();
  $xml->load($file);
...
function xmlimporter_readxml($file) {
  $xml = new DOMDocument();
  $xml->load($file);
  ...
  foreach($listings as $klisting => $listing) {
    list($success, $cat_info, $meta_info) = xmlimporter_ad($listing, $cat_info, $meta_info);

    if($success!=2) { //2 is the success code for active ads & 1 for inactive
      $errormsg .= sprintf(__("%s (Item %d)", "xmlimporter"), $success, $klisting)."<br/>";
    }

    $delete_images = glob(osc_content_path()."downloads/xmlimporterimage_*");
  ...
  if($errormsg!='') {
    ...
    osc_add_flash_ok_message(__('All ads were imported correctly', 'xmlimporter'), 'admin');
  }
}

function xmlimporter_adfromfile($file, $num_ad, $cat_info = array(), $meta_info = array()) {
  $xml = new DOMDocument();
  $xml->load($file);
  $listings = $xml->getElementsByTagName('listing');
  return xmlimporter_ad($listings->item($num_ad), $cat_info, $meta_info);
}

function xmlimporter_ad($listing, $cat_info, $meta_info) {
  ...
  foreach($image_list as $k=>$image) {
    $tmp_name = "xmlimporterimage_".$k.'_'.microtime();
...
osc_add_hook('admin_header','xmlimporter_admin_menu');