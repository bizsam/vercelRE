...
switch(Params::getParam("subaction")) {
  case 'parsead':
    ...
    if(is_numeric($num_ad) && $file!='') {
      ...
      list($success, $cat_info, $meta_info) = xmlimporter_adfromfile($file, $num_ad, $cat_info, $meta_info);
      echo json_encode(array('error' => $success, 'cat_info' => $cat_info, 'meta_info' => $meta_info));
    } else {
      echo json_encode(array('error' => 1, 'msg' => __('Invalid ad number', 'xmlimporter')));
    }
    break;

  default:
    echo json_encode(array('error' => 1, 'msg' => __('No action defined', 'xmlimporter')));
    break;
}