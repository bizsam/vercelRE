<?php
/*
  Plugin Name: Dating Attributes Plugin
  Plugin URI: https://osclasspoint.com/
  Description: Plugin extends a category of items to store dating attributes such as gender you're looking for and the type of relation.
  Version: 3.2.0
  Author: MB Themes
  Author URI: https://osclasspoint.com
  Author Email: info@osclasspoint.com
  Short Name: dating_attributes
  Plugin update URI: dating_attributes
  Support URI: https://forums.osclasspoint.com/free-plugins/
  Product Key: eT3rb042WfSXRc19otYi
*/
...
function dating_form($catId = '') {
  ...
  if($catId!="") {
    ...
    if(osc_is_this_category('dating_attributes', $catId)) {
...
function dating_search_form($catId = null) {
  ...
  if($catId!=null) {
    // We check if the category is the same as our plugin
    foreach($catId as $id) {
  		if(osc_is_this_category('dating_attributes', $id)) {
...
function dating_form_post($item) {
  ...
  if($catId!=null) {
    // We check if the category is the same as our plugin
    if(osc_is_this_category('dating_attributes', $catId) && $item_id!=null) {
...
function dating_item_detail() {
  if(osc_is_this_category('dating_attributes', osc_item_category_id())) {
    $dating_array = array(
